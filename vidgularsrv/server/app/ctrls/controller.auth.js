...
var createJWT = require('../core/auth.common').createJWT;
var mResponse = require('../core/response.common');

exports.postLogin = function(req, res) {
  req.app.db.models.User.findOne({ email: req.body.email }, '+password', function(err, user) {
    if (!user) {
    ...
    user.comparePassword(req.body.password, function(err, isMatch) {
      ...
      res.send({ success: true, token: createJWT(user) });
    });
  });
};

exports.postSignup = function(req, res) {
  req.app.db.models.User.findOne({ email: req.body.email }, function(err, existingUser) {
    if (existingUser) {
...
exports.postGoogle = function(req, res) {
  ...
  request.post(accessTokenUrl, { json: true, form: params }, function(err, response, token) {
    ...
    request.get({ url: peopleApiUrl, headers: headers, json: true }, function(err, response, profile) {
      ...
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ google: profile.sub }, function(err, existingUser) {
          if (existingUser) {
            return res.status(409).send({ message: 'There is already a Google account that belongs to you' });
          }
          var token = req.headers.authorization.split(' ')[1];
          var payload = jwt.decode(token, config.TOKEN_SECRET);
          req.app.db.models.User.findById(payload.sub, function(err, user) {
            if (!user) {
...
exports.postGithub = function(req, res) {
  ...
  request.get({ url: accessTokenUrl, qs: params }, function(err, response, accessToken) {
    ...
    request.get({ url: userApiUrl, qs: accessToken, headers: headers, json: true }, function(err, response, profile) {
      ...
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ github: profile.id }, function(err, existingUser) {
          if (existingUser) {
...
exports.postInstagram = function(req, res) {
  ...
  request.post({ url: accessTokenUrl, form: params, json: true }, function(error, response, body) {
    ...
    if (req.headers.authorization) {
      req.app.db.models.User.findOne({ instagram: body.user.id }, function(err, existingUser) {
        if (existingUser) {
...
exports.postLinkedin = function(req, res) {
  ...
  request.post(accessTokenUrl, { form: params, json: true }, function(err, response, body) {
    ...
    request.get({ url: peopleApiUrl, qs: params, json: true }, function(err, response, profile) {
      ...
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ linkedin: profile.id }, function(err, existingUser) {
          if (existingUser) {
...
exports.postLive = function(req, res) {
  async.waterfall([
    // Step 1. Exchange authorization code for access token.
    function(done) {
    ...
    function(profile) {
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ live: profile.id }, function(err, user) {
          if (user) {
            return res.status(409).send({ message: 'There is already a Windows Live account that belongs to you' });
          }
          var token = req.headers.authorization.split(' ')[1];
          var payload = jwt.decode(token, config.TOKEN_SECRET);
          req.app.db.models.User.findById(payload.sub, function(err, existingUser) {
            if (!existingUser) {
              return res.status(400).send({ message: 'User not found' });
            }
        ...
        req.app.db.models.User.findOne({ live: profile.id }, function(err, user) {
          ...
          var newUser = new req.app.db.models.User();
...
exports.postFacebook = function(req, res) {
  var fields = ['id', 'email', 'first_name', 'last_name', 'link', 'name', 'about','age_range','bio','birthday','gender'];
  ...
  request.get({ url: accessTokenUrl, qs: params, json: true }, function(err, response, accessToken) {
    ...
    request.get({ url: graphApiUrl, qs: accessToken, json: true }, function(err, response, profile) {
      ...
      console.log(profile);
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ facebook: profile.id }, function(err, existingUser) {
          if (existingUser) {
            var token = createJWT(existingUser);
            return res.send({ token: token });
          ...
          req.app.db.models.User.findById(payload.sub, function(err, user) {
            ...
            user.bio = profile.about || profile.bio;
...
exports.postYahoo = function(req, res) {
  ...
  request.post({ url: accessTokenUrl, form: formData, headers: headers, json: true }, function(err, response, body) {
    ...
    request.get({ url: socialApiUrl, headers: headers, json: true }, function(err, response, body) {
      ...
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ yahoo: body.profile.guid }, function(err, existingUser) {
          if (existingUser) {
  ...
  if (!req.body.oauth_token || !req.body.oauth_verifier) {
    ...
    request.post({ url: accessTokenUrl, oauth: accessTokenOauth }, function(err, response, accessToken) {
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          req.app.db.models.User.findOne({ twitter: profile.id }, function(err, existingUser) {
            if (existingUser) {
...
exports.postFoursquare = function(req, res) {
  ...
  request.post({ url: accessTokenUrl, form: formData, json: true }, function(err, response, body) {
    ...
    request.get({ url: profileUrl, qs: params, json: true }, function(err, response, profile) {
      ...
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ foursquare: profile.id }, function(err, existingUser) {
          if (existingUser) {
...
exports.postTwitch = function(req, res) {
  ...
  request.post({ url: accessTokenUrl, form: formData, json: true }, function(err, response, accessToken) {
   var params = {
    ...
    request.get({ url: profileUrl, qs: params, json: true }, function(err, response, profile) {
      // Step 3a. Link user accounts.
      if (req.headers.authorization) {
        req.app.db.models.User.findOne({ twitch: profile._id }, function(err, existingUser) {
          if (existingUser) {
...
exports.postUnlink = function(req, res) {
  ...
  req.app.db.models.User.findById(req.user, function(err, user) {
    if (!user) {
      return res.status(400).send({ message: 'User Not Found' });
    }
